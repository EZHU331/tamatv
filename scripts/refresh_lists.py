...
def fetch(url: str) -> str:
    return m3u.fetch_text(url, timeout=30)


def safe_https(url: str) -> str | None:
    cleaned = url.strip().split()[0].strip(".,;)\">'")
    if not urlcheck.is_safe_https_url(cleaned):
        return None
    try:
    ...
    if host not in harvest.CATALOG_HOSTS and not host.endswith(".github.io"):
        return None
    if ".." in parsed.path:
        return None
...
def safe_origin_href(url: str | None, fallback_name: str) -> str:
    ...
    if clean and GITHUB_REPO.match(clean.rstrip("/")):
        return clean.rstrip("/")
    if clean and urlparse_host(clean) in harvest.CATALOG_HOSTS:
        return clean
    return IPTV_ORG
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.catalog_only:
        ...
        country_rows, category_rows, epgshare, live_stats = build_live(
            country_rows, category_rows, args.probe, channels, logos
        )
